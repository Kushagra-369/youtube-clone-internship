                        ...
                        <div className="bg-linear-to-br from-gray-900/90 to-blue-950/90 p-6 sm:p-8 md:p-12 rounded-2xl sm:rounded-3xl border border-blue-500/30 text-center max-w-sm sm:max-w-md w-full">
                            ...
                            {callState.callStatus === "ringing" ? (
                                <div className="flex gap-3 sm:gap-6 justify-center flex-wrap">
                                    <button
                                        onClick={acceptCall}
                                        className="px-6 sm:px-8 md:px-10 py-3 sm:py-4 bg-linear-to-r from-green-600 to-emerald-600 rounded-xl sm:rounded-2xl font-semibold text-base sm:text-lg hover:from-green-500 hover:to-emerald-500 transition-all shadow-lg shadow-green-500/30 flex-1 sm:flex-none min-w-25"
                                    >
                                        Accept
                                    </button>
                                    <button
                                        onClick={rejectCall}
                                        className="px-6 sm:px-8 md:px-10 py-3 sm:py-4 bg-linear-to-r from-red-600 to-rose-600 rounded-xl sm:rounded-2xl font-semibold text-base sm:text-lg hover:from-red-500 hover:to-rose-500 transition-all shadow-lg shadow-red-500/30 flex-1 sm:flex-none min-w-25"
                                    >
                                        Reject
                                    </button>
                                </div>
                ...
                {callState.isInCall && (
                    <div className="fixed inset-0 bg-black z-40 flex flex-col">
                        ...
                        <div className="flex-1 relative p-1 sm:p-2 md:p-4 min-h-0">
                            {/* Remote Video - full size on all screens */}
                            <div className="w-full h-full relative bg-gray-900 rounded-xl sm:rounded-2xl overflow-hidden">
                                <video
                                    ref={remoteVideoRef}
                                    autoPlay
                                    playsInline
                                ...
                                <div className="absolute bottom-2 sm:bottom-4 left-2 sm:left-4 text-xs sm:text-sm text-gray-300 bg-black/60 px-2 sm:px-3 py-1 rounded-lg backdrop-blur-sm">
                                    {callerName}
                                </div>
                                {/* Call status indicator */}
                                <div className="absolute top-2 sm:top-4 left-1/2 -translate-x-1/2 text-xs sm:text-sm text-green-400 bg-black/60 px-3 sm:px-4 py-1 rounded-full backdrop-blur-sm flex items-center gap-1.5">
                            ...
                            {/* Local Video - PIP overlay on mobile, side-by-side on large screens */}
                            <div className={`
                                absolute rounded-xl sm:rounded-2xl overflow-hidden 
                                border-2 border-blue-500/60 shadow-lg shadow-blue-500/20
                                bg-gray-900
                                transition-all duration-300
                                /* Mobile: bottom-right PIP */
                                bottom-2 sm:bottom-4 right-2 sm:right-4
                                w-25 h-25 xs:w-[120px] xs:h-[170px] 
                                sm:w-37.5 sm:h-52.5 
                                md:w-45 md:h-62.5
                                lg:static lg:w-auto lg:h-auto lg:rounded-xl 
                            ...
                            `}>
                                ...
                                />
                                {/* Local user label */}
                                <div className="absolute bottom-1 sm:bottom-2 left-1 sm:left-2 text-[8px] xs:text-[10px] sm:text-xs text-gray-300 bg-black/60 px-1.5 sm:px-2 py-0.5 rounded backdrop-blur-sm flex items-center gap-1">
                                    You
                                    {callState.isMuted && <span>🔇</span>}
                                    {callState.isScreenSharing && <span>🖥️</span>}
                                </div>
                                {!localStreamRef.current && (
                                    <div className="absolute inset-0 flex items-center justify-center text-gray-500 text-2xl sm:text-3xl">
                                        📷
                                    </div>
                                )}
                            </div>
